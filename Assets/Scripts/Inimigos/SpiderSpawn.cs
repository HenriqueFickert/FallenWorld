...
public class SpiderSpawn : MonoBehaviour {
    ...
    public enum Estado { Patrulha = 1, Seguindo, Atacando, Morto, Teia, SeguirAranha}
    ...
    void Update()
    {
        ...
        if (!estaMorto)
        {
            ...
            float distanciaParaAAranha = Vector3.Distance(aranhaPai.position, transform.position);
            ...
            else if (distanciaParaAAranha <= raioDeVisao && estado != Estado.Atacando && estado != Estado.Teia && distanciaParaOJogador > raioDeAtaque)
            {
                estado = Estado.SeguirAranha;
            }
    ...
    void SeguindoAranha()
    {
        ...
        Ray ray = new Ray(transform.position, aranhaPai.transform.position - transform.position);
        RaycastHit hit;
        if (Physics.Raycast(ray, out hit, 25))
        {
            agent.SetDestination(aranhaPai.transform.position);
            Debug.DrawLine(ray.origin, hit.point);
        }
        else
        {
